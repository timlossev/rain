...
- **Refresh when rendering**: a checkbox re-runs that same webhook call
  every time the document's content is actually displayed -- its own
  page, or Home if also shown there -- not just on the manual button.
