...
| `cloud-environment-register.rain` | Cloud Environment asset type | CM-2/CM-8(3)/SI-7 (pairs with the pattern in [`docs/drift-detection-showcase.md`](../drift-detection-showcase.md)) |
...
| `fedramp-scn-fields.rain` | *Ticket* fields | Mirrors fedramp.gov's Significant Change Notification schema (2026-06-24) -- an SCN is structurally a Change ticket's own metadata (what changed, why, the plan and timeline), so this adds fields to Change tickets rather than a new asset type. Pairs with `fedramp-scn-export.jq`, below. |
...
`fedramp-scn-export.jq` is the same kind of packaged transformer,
this time for the *Tickets* JSON export -- a FedRAMP Significant
...
1. Import `fedramp-scn-fields.rain` (above); the fields show up on
   every ticket type (ticket-scoped custom fields apply tenant-wide in
   RAIN), but only fill them in on Change tickets that actually
   qualify as a significant change (SCN-CSO-EVA) -- routine recurring
   changes don't need one.
2. **Edit `fedramp-scn-export.jq` once before using it**: its own
   `certification_package_overview_uri` at the top is a per-CSP
   constant (the URL of your own Certification Package Overview
   document, see the section above), not per-change data. The schema
   requires it; left blank, the transformer omits the key rather than
   inventing a placeholder, which fails validation until it's filled
   in. Upload the edited file as a one-off on the export screen, or
   save it as a `.jq` Document to reuse it (Documents > New document >
   Type new content > `.jq`, paste in the edited contents).
