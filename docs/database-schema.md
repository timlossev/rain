...
| `document_acknowledgments` | (0048) One row per (`document_id`, `user_id`) who has clicked "I have read this" on that document's Acknowledgment tab, upserted so `acknowledged_at` always reflects the latest read, not the first. `ON DELETE CASCADE` on `document_id`. `user_id` is the same unenforced cross-schema `control.users` id as `owner_user_id`/`uploaded_by`. |
