...
- PL-4 / PS-6 (Rules of Behavior / Access Agreements) -- a document can require acknowledgment from a specific person or group, emailed when the requirement is set and tracked as pending (surfaced in the client portal) until each of them clicks "I have read this," timestamped per user -- the assigned, followed-up read-and-attest record these controls ask for when the document in question is a rules-of-behavior or access-agreement document, not just an open-ended "someone may have read this."
...
- CM-2 (Baseline Configuration), with a CM-8(3) (Automated Unauthorized Component Detection) / SI-7 (Software, Firmware, and Information Integrity) flavor -- a document populated from a scheduled infrastructure-discovery run (Terraform + a discovery tool against the live account) alerts the moment two consecutive snapshots differ, the same diff-on-refresh mechanism every webhook-populated document already has (see "Infrastructure drift detection" in `docs/user-guide.md`). This is detection of undocumented change, not the CM-3 approval trail itself (already in the direct 33) -- the two together are what "every change was both approved and is the only thing that happened" actually takes. A starter Cloud Environment bundle template (`docs/compliance-templates/cloud-environment-register.json`) tracks the account/environment this applies to.
